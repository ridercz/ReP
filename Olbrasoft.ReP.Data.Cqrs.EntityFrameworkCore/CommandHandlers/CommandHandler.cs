...
public abstract class CommandHandler<TEntity, TCommand, TResult> : IRequestHandler<TCommand, TResult>
    where TCommand : BaseCommand<TResult> where TEntity : class
{
    ...
    protected CommandHandler(RepDbContext context)
    {
    ...
    protected virtual async Task<TEntity?> SingleOrDefaultAsync(Expression<Func<TEntity, bool>> predicate, CancellationToken cancellationToken = default)
    {
    ...
    protected async Task<bool> AnyAsync(Expression<Func<TEntity, bool>> predicate, CancellationToken token = default)

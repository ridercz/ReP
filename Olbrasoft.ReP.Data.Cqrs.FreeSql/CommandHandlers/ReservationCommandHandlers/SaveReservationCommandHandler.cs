...
{
    public SaveReservationCommandHandler(IMapper mapper, IConfigure<Reservation> configurator, RepDbContextFreeSql context) 
        : base(mapper, configurator, context)
    {
    }
    ...
    protected override async Task<SaveReservationCommandResult> GetResultToHandleAsync(SaveReservationCommand command, CancellationToken token)
    {
        ...
        else
        {
            ...
            command.Status = await UpdateAndSaveAsync(reservation, token);
        }
